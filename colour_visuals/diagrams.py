...
from colour.hints import ArrayLike, Optional, Sequence, Type, cast
...
class VisualSpectralLocus2D(gfx.Group):
    """
    Create a *Spectral Locus* 2D visual.
    """
    ...
    def __init__(
        ...
        if colors is None:
            colors_w = lines_w["colour"]
        else:
            colors_w = np.tile(colors, (positions.shape[0], 1))

        self._wavelengths = gfx.Line(
            gfx.Geometry(
                positions=as_contiguous_array(positions),
                colors=as_contiguous_array(
                    append_alpha_channel(colors_w, opacity)
                ),
            ),
            gfx.LineSegmentMaterial(thickness=thickness, color_mode="vertex"),
        )
        ...
        if colors is None:
            colors_lp = lines_w["colour"][::2]
        else:
            colors_lp = np.tile(colors, (positions.shape[0], 1))
        ...
        self._points = gfx.Points(
            gfx.Geometry(
                ...
                colors=as_contiguous_array(
                    append_alpha_channel(colors_lp, opacity)
                ),
            ),
...
class VisualSpectralLocus3D(gfx.Line):
    """
    Create a *Spectral Locus* 3D visual.
    """
    ...
    def __init__(
        ...
        positions = colourspace_model_axis_reorder(
            XYZ_to_colourspace_model(
                cmfs.values, colourspace.whitepoint, colourspace_model
            ),
            colourspace_model,
        )
        positions = np.concatenate(
        ...
        if colors is None:
            colors = XYZ_to_RGB(cmfs.values, colourspace)
            colors = np.concatenate([colors[:-1], colors[1:]], axis=1).reshape(
                [-1, 3]
            )
        else:
...
class VisualChromaticityDiagramCIE1931(gfx.Group):
    ...
    def __init__(
        self,
        kwargs_visual_chromaticity_diagram: Optional[dict] = None,
        kwargs_visual_spectral_locus: Optional[dict] = None,
    ):
        super().__init__()
...
if __name__ == "__main__":
    ...
    visual_3 = VisualChromaticityDiagramCIE1931(
        kwargs_visual_chromaticity_diagram={"colors": [0.5, 0.5, 0.5]}
    )
    ...
    visual_6 = VisualSpectralLocus2D(colors=[0.5, 0.5, 0.5])
    ...
    visual_8 = VisualSpectralLocus3D(colors=[0.5, 0.5, 0.5])
    visual_8.local.position = np.array([5, 0, 0])
    scene.add(visual_8)

    visual_9 = VisualSpectralLocus3D(colourspace_model="CIE XYZ")
